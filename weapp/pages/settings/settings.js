...
Page({
  ...
  setDisplayStatus(display, extra) {
    this.setData(Object.assign({
      ...
      showRebindActions: ["RELOGIN_REQUIRED", "CAPTCHA_REQUIRED"].includes(display.status)
    }, extra || {}));
  },
  ...
  handleBindFailure(err) {
    ...
    if (wasBound && isTransientBindError(err)) {
      this.setDisplayStatus(deriveStatus({ bound: true, error: errorCode(err) }));
      wx.showToast({ title: userErrorMessage(err, "学校系统暂时无法访问，请稍后再试"), icon: "none" });
      return;
    }
  ...
  unbindAccount() {
    ...
    wx.showModal({
      title: "确认解除绑定",
      content: "解除后将删除校园账号密码和登录状态，无法继续自动同步；历史课表和成绩仍会保留在账号中。",
      confirmText: "解除绑定",
      confirmColor: "#e74c3c",
      success: result => {
  ...
  doUnbindAccount() {
    ...
    api.post("/unbind-account", {}, { timeout: 30000 }).then(data => {
      wx.hideLoading();
      this.setData({ unbinding: false, password: "", studentId: "", privacyAccepted: Boolean(wx.getStorageSync("privacyAccepted")), bindingSuccess: false, maskedStudentId: "" });
