(function(){
  ...
  var friendlyErrorCategories={
    ...
    CAMPUS_LOGIN_REQUIRED:"用户需要先绑定账号",
    ...
    RATE_LIMITED:"操作过于频繁",
    REVIEW_DEMO_ACCOUNT_CONFLICT:"用户需要先解绑账号",
    REVIEW_DEMO_UNAVAILABLE:"系统问题，需要关注",
    TIMETABLE_CONFIG_FAILED:"系统问题，需要关注",
    TIMETABLE_EMPTY:"暂时没有可用数据",
    TIMETABLE_TODAY_FAILED:"学校系统问题",
    TIMETABLE_WEEK_FAILED:"学校系统问题",
    UNAUTHORIZED:"用户需要重新登录",
    UNBIND_FAILED:"系统问题，需要关注",
    UNKNOWN:"暂时无法判断",
    WECHAT_CONFIG_MISSING:"系统问题，需要关注",
    WECHAT_LOGIN_FAILED:"微信登录问题",
  ...
  function getFriendlyErrorInfo(errorType){var code=String(errorType||"UNKNOWN").trim().toUpperCase();return{code:code,reason:friendlyErrorReasons[code]||"暂时无法识别的问题",category:friendlyErrorCategories[code]||"暂时无法判断"};}
  function countText(value,unit){var number=Number(value);return (Number.isFinite(number)?number:0)+" "+unit;}
  function bytesText(value){var number=Number(value);return (Number.isFinite(number)&&number>=0?number/1048576:0).toFixed(1)+" MB";}
  function percentText(value){var number=Number(value);return (Number.isFinite(number)?number:0).toFixed(1)+"%";}
  ...
  function renderBindingFunnel(value){var funnel=value||{};var rates=funnel.conversionRates||{};text("bindStarted",countText(funnel.started,"次"));text("bindPortalConfirmed",countText(funnel.portalConfirmed,"次"));text("bindSaved",countText(funnel.saved,"次"));text("bindJwxtConfirmed",countText(funnel.jwxtConfirmed,"次"));text("bindPortalRate","上一步转化率 "+percentText(rates.portalFromStarted));text("bindSavedRate","上一步转化率 "+percentText(rates.savedFromPortal));text("bindJwxtRate","上一步转化率 "+percentText(rates.jwxtFromSaved));text("bindFinalRate",percentText(rates.finalSuccess));}
  function renderBindingFailures(rows){var list=el("bindingFailureList");list.replaceChildren();if(!rows||!rows.length){var empty=document.createElement("p");empty.className="binding-empty";empty.textContent="今天暂时没有绑定失败";list.append(empty);return;}rows.forEach(function(item){var row=document.createElement("div");row.className="binding-failure";var reason=document.createElement("strong");reason.textContent=item.reason||"其他";var detail=document.createElement("span");detail.textContent=countText(item.failureCount,"次")+" · 影响 "+countText(item.affectedUsers,"人");row.append(reason,detail);list.append(row);});}
  function renderFeatureRanking(rows){var list=el("featureRanking");list.replaceChildren();if(!rows||!rows.length){var empty=document.createElement("p");empty.className="empty";empty.textContent="今天暂时没有核心功能调用";list.append(empty);return;}rows.forEach(function(item,index){var row=document.createElement("div");row.className="ranking-item";var rank=document.createElement("span");rank.className="ranking-index";rank.textContent="#"+(index+1);var label=document.createElement("span");label.textContent=item.label;var count=document.createElement("strong");count.className="ranking-count";count.textContent=countText(item.total,"次");row.append(rank,label,count);list.append(row);});}
  function renderSummary(data){
    var today=data.today||{};var status=today.httpStatus||{};var todayHasRequests=Number(today.requestCount)>0;text("uniqueUsers",countText(today.uniqueUsers,"人"));text("activeUsers",countText(today.activeUsers5m,"人"));text("activeUsersHelp",Number(today.activeUsers5m)===0?"最近5分钟暂时没人操作":"最近5分钟内产生核心功能调用的匿名用户");text("requestCount",countText(today.requestCount,"次"));text("http5xxToday",countText(today.http5xxToday,"次"));el("http5xxCard").className=Number(today.http5xxToday)>0?"metric alert":"metric";text("http2xxToday",countText(status.http2xx,"次"));text("http3xxToday",countText(status.http3xx,"次"));text("http4xxToday",countText(status.http4xx,"次"));text("http5xxDistribution",countText(status.http5xx,"次"));text("averageMs",countText(today.averageResponseTimeMs,"毫秒"));text("p95Ms",countText(today.p95ResponseTimeMs,"毫秒"));text("averageSpeed",speedText(today.averageResponseTimeMs,todayHasRequests));text("p95Speed",speedText(today.p95ResponseTimeMs,todayHasRequests));
    renderEventGrid("eventGrid",data.events,false);renderFeatureRanking(data.featureRanking);renderBindingFunnel(data.bindingFunnel);renderBindingFailures(data.bindingFailures);var lifetime=data.lifetime||{registeredUsers:0,boundUsers:0,bindingRate:0,requestCount:0,averageResponseTimeMs:0,p95ResponseTimeMs:0,monitoringStartedAt:null,events:{}};var lifetimeHasRequests=Number(lifetime.requestCount)>0;text("registeredUsers",countText(lifetime.registeredUsers,"人"));text("boundUsers",countText(lifetime.boundUsers,"人"));text("bindingRate",percentText(lifetime.bindingRate));text("lifetimeRequestCount",countText(lifetime.requestCount,"次"));text("lifetimeAverageMs",countText(lifetime.averageResponseTimeMs,"毫秒"));text("lifetimeP95Ms",countText(lifetime.p95ResponseTimeMs,"毫秒"));text("lifetimeAverageSpeed",speedText(lifetime.averageResponseTimeMs,lifetimeHasRequests));text("lifetimeP95Speed",speedText(lifetime.p95ResponseTimeMs,lifetimeHasRequests));renderEventGrid("lifetimeEventGrid",lifetime.events,true);if(lifetime.monitoringStartedAt){var startedAt=new Date(lifetime.monitoringStartedAt);var generatedAt=new Date(data.generatedAt);var days=Math.max(1,Math.ceil((generatedAt.getTime()-startedAt.getTime())/86400000));text("monitoringDays",countText(days,"天"));text("monitoringStart","数据开始记录时间："+startedAt.toLocaleString("zh-CN",{timeZone:"Asia/Shanghai",hour12:false}));}else{text("monitoringDays","0 天");text("monitoringStart","数据开始记录时间：尚无数据");}
  }
  function renderChart(data){
    ...
    function pathFor(key){return points.map(function(p,index){var x=pad+index*(width-pad*2)/Math.max(1,points.length-1);var y=height-pad-(Number(p[key])||0)*(height-pad*2)/Math.max(1,max);return(index?"L":"M")+x.toFixed(1)+" "+y.toFixed(1);}).join(" ");}var totalPath=pathFor("requestCount");var area=document.createElementNS("http://www.w3.org/2000/svg","path");area.setAttribute("d",totalPath+" L "+(width-pad)+" "+(height-pad)+" L "+pad+" "+(height-pad)+" Z");area.setAttribute("class","chart-area");svg.append(area);[["requestCount","total"],["http2xx","http-2xx"],["http4xx","http-4xx"],["http5xx","http-5xx"]].forEach(function(item){var stroke=document.createElementNS("http://www.w3.org/2000/svg","path");stroke.setAttribute("d",pathFor(item[0]));stroke.setAttribute("class","chart-line "+item[1]);svg.append(stroke);});
  }
  ...
  function renderHealth(data){var serviceState=data.service&&data.service.status;var serviceOk=serviceState==="ok";var runtime=data.runtime||{};text("expressStatus",healthLabel(serviceState));el("expressStatus").className=serviceOk?"ok":serviceState==="error"?"bad":"";text("serviceUptime",durationText(runtime.processUptimeSeconds!==undefined?runtime.processUptimeSeconds:data.service&&data.service.uptimeSeconds));text("runtimeRss",bytesText(runtime.rssBytes));text("runtimeHeapUsed",bytesText(runtime.heapUsedBytes));text("runtimeHeapTotal",bytesText(runtime.heapTotalBytes));text("eventLoopLag",countText(runtime.eventLoopLagMs,"毫秒"));var databaseState=data.postgres&&data.postgres.status;var databaseOk=databaseState==="ok";text("postgresStatus",healthLabel(databaseState));el("postgresStatus").className=databaseOk?"ok":databaseState==="error"?"bad":"";text("postgresLatency",data.postgres?countText(data.postgres.latencyMs,"毫秒")+" · "+speedText(data.postgres.latencyMs,true):"暂时无法确认");}
